...
#define OUTBUF_INITIAL_CAP 16384
...
static void outbuf_append_line_highlighted(outbuf_t *ob, int color_enabled, const char *line, size_t line_len, size_t match_start, size_t match_end) {
    if (match_start <= line_len && match_end <= line_len && match_start < match_end) {
        ...
        if (color_enabled) outbuf_append_str(ob, ANSI_MATCH);
        outbuf_append(ob, line + match_start, match_end - match_start);
        if (color_enabled) outbuf_append_str(ob, ANSI_RESET);
...
int greg_search_file(const char *filepath, greg_matcher_t *matcher, pcre2_match_data *match_data, greg_printer_t *printer, const greg_options_t *opts) {
    ...
    // 1. Binary check (scans up to 8KB)
    if (!opts->search_binary && size > 0) {
        size_t scan_len = (size < 8192) ? size : 8192;
    ...
    // 2. Whole file match pre-check (skip files without any match)
    if (!opts->invert_match && size > 0) {
        size_t dummy_start, dummy_end;
        int rc = greg_matcher_find(matcher, match_data, buffer, size, 0, &dummy_start, &dummy_end);
        if (rc == 0) {
            greg_file_unmap(&view);
            return 0;
        }
        if (rc > 0 && opts->files_with_matches) {
    ...
    // 3. Local match tracking buffer optimization
    ...
    // 3. Line scanning loop. Uses memchr.
    while (offset < size) {
        ...
        if (is_match) {
            ...
            // Early stopping for -m / --max-count
        ...
        if (outbuf_init(&ob) == 0) {
            if (opts->heading) {
                if (printer->color_enabled) {
                    ...
                    outbuf_append_str(&ob, ANSI_RESET "\n");
                } else {
                    outbuf_append_str(&ob, filepath);
            ...
            if (opts->raw) {
                ...
                for (int i = 0; i < match_count; i++) {
                    if (printer->show_line_numbers) {
                        if (printer->color_enabled) {
                            outbuf_append_str(&ob, "  " ANSI_LINENUM);
                            outbuf_append_size(&ob, matches[i].line_num);
                            outbuf_append_str(&ob, ":" ANSI_RESET " ");
                        } else {
                            outbuf_append_str(&ob, "  ");
